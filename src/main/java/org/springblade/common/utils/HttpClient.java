...
public class HttpClient {
    ...
    public static Map<String, String> convertMap(Map<String, Object> objectMap) throws Exception {
        ...
        for (Map.Entry<String, Object> entries : objectMap.entrySet()) {
            map.put(entries.getKey(),
                    null == entries.getValue() ? null : String.valueOf(entries.getValue()));
        }
        return map;
    }

    /**
     * 发送get请求，接收json响应数据 GET
     *
    ...
    public static JSONObject doGet(String url, Map<String, String> param) {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                throw new HttpClientException("请求失败 状态码：{}",
    ...
    /**
     * 发送post请求，上传byte POST binary
     *
     * @param url 请求地址，不拼接
     * @return
     */
    public static JSONObject doPostBinaryBody(String url, byte[] bytes, String fileName)
        ...
        try {
            ...
            multipartEntityBuilder.addBinaryBody("video", bytes, ContentType.MULTIPART_FORM_DATA,
                    fileName);
    ...
    /**
     * 发送post请求，form-data数据传输 POST multipart/form-data
     *
     * @param url 请求地址
     * @return 返回json数据
     */
...
//            httpPost.setHeader("Content-Type", "multipart/form-data;charset=utf-8"); // 报错
            ...
            if (formData != null) {
                ...
                for (String key : formData.keySet()) {
                    multipartEntityBuilder.addTextBody(key, formData.get(key),
    ...
    /**
     * 发送post请求，接收json响应数据 POST application/x-www-form-urlencoded
     *
     * @param url   请求地址，不拼接
    ...
    /**
     * 发送post请求，接收json响应数据 POST application/json
     *
     * @param url  请求地址
     * @param json json入参
     * @return
